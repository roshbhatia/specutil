...
what-changes, and tasks grouped by phase with their done state. The projection
MUST be pure and deterministic: identical inputs yield byte-identical output.
...
- **THEN** the output contains one entry per change, keyed by change name, with
  ...
  ordered list of phases each containing ordered task items with text and done
  state
